...
【重要】引用标注要求：
- 当你使用了 rag_retrieve 或 search_posts 工具返回的资料时，请在回答中用 [1] [2] 等数字标注引用来源
- 编号与工具返回结果的顺序一致
- 引用标注放在句子末尾的句号之前，如："React.memo 可以优化性能[1]。"
- 同一资料被多处引用时使用相同编号"""
...
调用 rag_retrieve 时：
- 如果用户问题涉及具体知识点或需要查找文章内容，优先使用 rag_retrieve 做语义检索
- 检索到相关内容后，回答中用 [n] 标注引用来源（n 为返回结果的序号，从 1 开始）"""
