...
export function PostCard({
  ...
  const author =
    typeof post.author === "string" ? post.author : post.author?.display_name;
      ...
      {!isDeleted && showStats && (
        <View style={styles.cardStats}>
          <Text style={styles.statText}>
            {t("点赞")} {post.like_count ?? 0}
          </Text>
          <Text style={styles.statText}>
            {t("评论")} {post.comment_count ?? 0}
          </Text>
          <Text style={styles.statText}>
            {t("收藏")} {post.favorite_count ?? 0}
          </Text>
        </View>
      )}
    </>
