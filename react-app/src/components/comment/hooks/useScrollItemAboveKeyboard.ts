...
export function useScrollItemAboveKeyboard<ItemT>() {
  ...
  const registerItemLayout = useCallback(
    (itemId: number, event: LayoutChangeEvent) => {
  ...
  const handleBottomAccessoryLayout = useCallback(
  ...
  const scrollItemAboveKeyboard = useCallback(
    (itemId: number, options: ScrollItemOptions) => {
      const gap = options.gap ?? 12;
      const layout = itemLayouts.current.get(itemId);
      if (layout && listViewportHeight > 0) {
        const targetOffset = Math.max(
          0,
          layout.y + layout.height - listViewportHeight + gap,
        );
