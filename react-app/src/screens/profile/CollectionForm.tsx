    ...
    <View style={styles.authPromptCard}>
      {!isOpen ? (
        <Pressable
          ...
          onPress={onOpen}
        >
          <Text style={styles.actionButtonText}>{t("创建合集")}</Text>
        </Pressable>
      ) : (
        <>
          <Text style={styles.authPromptTitle}>
            {editingCollection ? "编辑合集" : "创建合集"}
          </Text>
          <TextInput
            style={styles.input}
            placeholder={t("合集名称")}
          ...
          <TextInput
            ...
            multiline
            placeholder={t("添加描述，让读者知道这个文件夹的主题")}
            placeholderTextColor="#a89994"
            value={description}
          ...
          <View style={styles.actionRow}>
            <Pressable style={styles.actionButton} onPress={onCancel}>
              <Text style={styles.actionButtonText}>{t("取消")}</Text>
            </Pressable>
            <Pressable
              style={[styles.actionButton, styles.actionButtonActive]}
              onPress={onSubmit}
